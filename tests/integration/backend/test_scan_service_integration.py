...
from vault_shared.db.models import ConnectorProvider, ScanStatus, ScanType
...
@requires_infra
def test_start_scan_creates_a_pending_job_for_a_connected_connector(db: Session) -> None:
    ...
    job = service.start_scan(
        connector.id, organization_id=user.organization_id, user_id=user.id
    )
...
@requires_infra
def test_start_scan_rejects_a_second_job_while_one_is_active(db: Session) -> None:
    user = _provision_user(db)
    connector = _provision_connected_connector(
        db, organization_id=user.organization_id, user_id=user.id
    )
    service = ScanService(db)
    service.start_scan(connector.id, organization_id=user.organization_id, user_id=user.id)
...
def test_get_owned_rejects_a_job_from_another_organization(db: Session) -> None:
    ...
    service = ScanService(db)
    job = service.start_scan(connector.id, organization_id=user.organization_id, user_id=user.id)
